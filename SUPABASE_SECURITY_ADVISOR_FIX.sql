...
create policy "payments_select_staff_registrations" on public.registrations for select to authenticated using (private.is_admin(auth.uid()) or private.is_staff(auth.uid()));

drop policy if exists "payments_select_staff_payment_events" on public.payment_events;
create policy "payments_select_staff_payment_events" on public.payment_events for select to authenticated using (private.is_admin(auth.uid()) or private.is_staff(auth.uid()));
