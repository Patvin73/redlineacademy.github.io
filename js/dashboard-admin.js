...
(function () {
  ...
  const NEW_I18N_KEYS = {
    id: {
      ...
      adMetricRevenue:       "Total Pendapatan (AUD)",
    ...
    en: {
      ...
      adMetricRevenue:       "Total Revenue (AUD)",
    ...
    try {
      ...
      data.forEach((s) => {
        ...
        tr.innerHTML = `
          ...
          <td><span class="ad-tag ad-tag--orange">${s.last_accessed_at ? timeAgo(s.last_accessed_at) : "Never"}</span></td>
          <td>
  ...
  async function loadStudentsTable() {
    ...
    try {
      ...
      if (empty) empty.style.display = "none";
      tbody.querySelectorAll("tr.ad-student-row").forEach((r) => r.remove());
      ...
      enrollments.forEach((en) => {
        ...
        const statusTag = en.status === "completed"
          ? `<span class="ad-tag ad-tag--green">Completed</span>`
          : en.status === "active"
          ? `<span class="ad-tag ad-tag--blue">Active</span>`
    ...
    } catch { if (empty) { empty.style.display = "table-row"; empty.querySelector("td").setAttribute("data-i18n", "lmsNoStudents"); } }
  }
    ...
    try {
      ...
      users.forEach((u) => {
        ...
        tr.innerHTML = `
          ...
          <td><span class="ad-tag ${u.is_active ? "ad-tag--green" : "ad-tag--gray"}">${u.is_active ? "Active" : "Inactive"}</span></td>
  ...
  async function loadEnrollmentsTable() {
    ...
    try {
      const { data } = await window.lmsSupabase
        .from("payments")
        .select(`
          id, amount, currency, payment_method, status, paid_at,
          profiles ( full_name ),
          courses   ( title )
        `)
      ...
      data.forEach((p) => {
        ...
        const statusTag = {
          completed: `<span class="ad-tag ad-tag--green">Paid</span>`,
          pending:   `<span class="ad-tag ad-tag--orange">Pending</span>`,
          failed:    `<span class="ad-tag ad-tag--red">Failed</span>`,
          refunded:  `<span class="ad-tag ad-tag--gray">Refunded</span>`,
        ...
        tr.innerHTML = `
          ...
          <td><strong>$${parseFloat(p.amount || 0).toFixed(2)} ${p.currency || "AUD"}</strong></td>
          <td>${statusTag}</td>
      ...
      if ($("payRevenue")) $("payRevenue").textContent = "$" + totalRevenue.toFixed(2);
